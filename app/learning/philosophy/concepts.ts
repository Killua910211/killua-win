...
export const concepts: Concept[] = [
  ...
  },
  {
    ...
    short:
      '判断某个东西处在什么心理状态，看的不是它由什么材料做成，而是这个状态在因果链条里占的位置：什么输入引起它，它又引起哪些别的内部状态和哪些行为。位置对上了就算同一种状态，所以同一种状态可以由不同材料实现。',
    detail:
    ...
    distinctions: [
      ...
      {
        from: '个例同一论',
        note: '这一条容易被漏掉，漏掉就会以为「同一论、功能主义、还原论」排成一条由强到弱的直线。功能主义与类型同一论冲突，与个例同一论并不冲突——否认「某一类心理状态整体对应某一类脑状态」，不等于否认「这一次的这个疼就是这一段具体的物理过程」。所以接受功能主义的人，仍然可以在个例这一层上是同一论者。',
      },
      {
        from: '行为主义',
