...
export const coreEntryLedgers: Record<string, CoreEntryLedger> = {
  ...
  'pt-freedom': {
    ...
    boundaries: [
      { kind: '概括', text: '决定论指在既定过去和自然规律下，未来只有一种展开方式；它不同于宿命论，也不等于“原因已经被科学完全发现”。相容论主张，责任所要求的那种行动控制与这样的决定论可以并存；不相容论则主张，真正要紧的那种自由不可能与它并存。双方争的不是“人有没有在选择的感觉”，而是这种控制究竟要求什么。', sourceIds: ['FRE-1', 'FRE-2'] },
      { kind: '概括', text: '“道德责任”也不是一个问题，至少要分成三问，这里先把三个名称的意思说定：可归属性问的是这件事算不算此人自己做的、是否反映了他本人的品格与在乎的东西；可要求说明问的是能不能要求他为这样做给出理由；可追究性问的是能不能据此责备他、向他提出要求或让他承担后果。三者许可的反应不同，所需的控制条件也未必相同——一个行为确实出自某人的品格，他是否应被追究仍要另行判断。看到一次错误就直接跳到“此人应受惩罚”，等于把这三问一次跨过。', sourceIds: ['FRE-2'] },
    ],
    objections: [
    ...
    review: { mode: '自审（尚未独立复审）', checkedOn: '2026-09-11', findings: [
      ...
      { location: '定义与边界｜责任三分；有力反对及回应｜运气与“回卷”论证', issue: '“可归属性／可追究性／可要求说明”三个名称直接抛出即被用于推理（说它们所需控制条件不同），却从未界定；“回卷”论证同样只有名称没有内容；“相容论问这样的决定论是否仍允许……”把一个立场写成了只在提问。', evidence: 'FRE-2 §3.1.1、§3.1.3 是三分的来源，FRE-1 §3.1 是运气与回卷论证的来源；本轮按内容整改标准第 2 条（术语在被用于推理前必须说清）与第 1 条逐段复查。', revision: '在“定义与边界”写出三问各问什么，并补一句它们可以分离；在运气论证一段写出回卷的设想方式，并为“自由意志论”补上界定；“相容论问……”改为“相容论主张……”。' },
  ...
  'pt-logic': {
    ...
    boundaries: [
      { kind: '概括', text: '有效（valid）与健全（sound）不同：有效说的是这个论证的形式保住真——前提若都为真，结论就不可能假；健全则在有效之外还要求前提确实为真。另有两个词也要分开。可推导性问的是：在一套给定的推演规则里，能不能一步步把结论推出来。语义有效性问的是：在任何一种为语言中各词项指派意义的方式（这样一种指派就叫一个“解释”或“模型”）之下，只要前提全真，结论是否都不可能假。前者是按规则一步步走，后者是把所有可能的赋义一并考虑。两者是否总是重合——推得出来的是不是都有效、有效的是不是都推得出来——属于元逻辑问题，也就是拿这套逻辑系统本身当研究对象的问题。', sourceIds: ['LOG-1'] },
    ...
    review: { mode: '自审（尚未独立复审）', checkedOn: '2026-09-11', findings: [
      ...
      { location: '定义与边界｜可推导性与语义有效性；容易混淆的地方｜因果与形式化', issue: '“语义有效性则以解释或模型刻画”用未解释的术语解释术语；“元逻辑”“对象语言”同样只有名称；因果那条里的“要估的量”“无偏”是未翻译的行话；归纳一段结尾“把归纳压成一句「关键检验」最容易失手”既生硬又指代不明。', evidence: 'LOG-1 §3 是推导、§4 是语义、§5 是二者关系；LOG-8 §2–§4 是因果假设与可识别性。按内容整改标准第 2、5 条逐句复查。', revision: '把可推导性与语义有效性各写成一句可读的说明，并点出“解释／模型”即对词项的一种赋义；元逻辑、对象语言就地解释；“要估的量”“无偏”改为普通说法；归纳一段结尾改为平实表述。' },
  ...
  'pt-language-meaning': {
    ...
    boundaries: [
      { kind: '概括', text: '词的意义、句子的真值条件、说话者意图和话语效果是四件不同的事。这里的“真值条件”指的是：一句话要在什么情况下才算真。说清“冰箱里有牛奶”在什么情况下为真，和说清某人这次说它是想达成什么、实际又造成了什么，并不是同一项工作。一个词可以有稳定的用法，却在不同语境中指向不同对象；一句在事实上没说错的话，也可能在特定场合造成误导。', sourceIds: ['LAN-1', 'LAN-2'] },
    ...
    objections: [
      { kind: '概括', text: '一条路线把意义主要交给指称——指称说的是一个词跟世界上哪个对象或哪类事物挂钩，“这个词指的是哪样东西”。它的难处在于：“孙悟空”没有对应的对象，“三”不是可以指给人看的东西，“时间是一条河”并不是在报告某条河，而“不该说谎”这类规范性语言（即说什么该做、什么算对错好坏的话）也不像是在指认什么。另一条路线把意义主要交给使用：一个词的意思就是它在实践中被怎样用。它要回答的则是另一组问题：既然对错由用法决定，那么“你这个词用错了”“这种说法本身有问题”凭什么还能成立，真理又为什么不只是某个群体碰巧形成的习惯。比较两条路线时该问的是：各自把哪些现象解释掉了，又把哪些现象留给了别人。', sourceIds: ['LAN-1', 'LAN-2'] },
    ...
    review: { mode: '自审（尚未独立复审）', checkedOn: '2026-09-11', findings: [
      ...
      { location: '定义与边界｜真值条件与真理承担者；有力反对及回应｜两条路线', issue: '“真值条件”“真理承担者”“指称”“规范性语言”都在被用于推理前没有解释；“选择何者会影响对应、紧缩和语义理论的表达”更是用三个未解释的理论名去解释一个未解释的术语。', evidence: 'LAN-2 的定位（§1.1 对应论、§6.1 真理承担者、§6.2 真理与真值条件）支持承担者之分与对应论，但不覆盖紧缩论；LAN-1 §1.2 分列各类词义理论。按内容整改标准第 2 条复查。', revision: '真值条件就地用“冰箱里有牛奶”说明；真理承担者的辨析另立一段、标为解释性重构且不挂来源，源自 LAN-2 的部分仍作概括并去掉无从核对的“紧缩”提法；指称与规范性语言在“有力反对及回应”第一段就地解释，两条路线各自要回答的问题也写具体。' },
  ...
  'pt-western-modern': {
    ...
    review: { mode: '自审（尚未独立复审）', checkedOn: '2026-09-11', findings: [
      ...
      { location: '时段、标签与材料边界；史学分类的争议', issue: '“先天”在三个论题名里反复出现却从未解释；“按‘某一主题领域 S’来表述”是照搬综述措辞，中文读者读不出它在说什么；“而这被认为是误读”主语不明，看不出是谁作此判断。', evidence: 'MOD-1 §2–§4 把三论题都相对于主题领域表述，§1 指出把洛克、休谟、里德讲成接力属误读。按内容整改标准第 2、5 条复查。', revision: '就地解释“先天”；“某一主题领域 S”改写为“在某一类主题上”并补数学与物体知识的例子；“被认为是误读”改为点明是相关综述作此判断；另把“经验主义不等于只相信眼睛”一条补足内容。' },
  ...
  'pt-confucian': {
    status: '核验正文｜自审完成',
    scope: '本页只走先秦这一段，主要比较孔子相关文本、孟子与荀子在修养和人性上的分歧。“儒家”一词跨两千年，指过学派、经学、教育制度也指过国家意识形态，读的时候要随时问它在哪一层上说话。礼、孝、性善这些词在原文里各有论证支撑，一旦压成“服从”“乐观”一类标签，剩下的就只有标签，论证全丢了。',
    ...
    objections: [
      ...
      { kind: '概括', text: '荀子把自然欲望、有意的努力（wei）、师法与礼联系起来，能解释为何德性需要学习：冲动本身并不可鄙，问题出在不经反省的发作会造成伤害，礼与乐提供了可用的出口。批评者则问这种秩序如何避免把既有权威定为标准——而“道”究竟是圣人发现的、还是人所构造的，正是荀子研究里尚无定论的争议：有一段文本说“道”不是天之道、也不是地之道，而是人所行的道；另一些段落又要人观察四时运行的规律。不能由“性恶”推出人人天生邪恶，也不能因为礼有其历史来由就让它免受批评。', sourceIds: ['CON-3', 'CON-4'] },
    ],
    confusions: [
    ...
    review: { mode: '自审（尚未独立复审）', checkedOn: '2026-09-11', findings: [
      ...
      { location: '内部论辩与当代质问｜孟子一段；文本、术语与传承边界', issue: '“道德端绪”的“端”从未解释即进入论证；“评价仍须说明教育与物质条件为何能支持或损坏养成”主语不明；“把两千年实践倒灌进《论语》”是生硬比喻；荀子一段“道非天之道，乃人之所以道也”以引号形式出现，其措辞与通行文本对不上。', evidence: 'CON-2 §3 是人性之善与四端的来源；CON-4 §5 记录的是“道非天之道”一段与强调观察时序的段落之间的解释争论，本条无法核对该句的校勘文字。按内容整改标准第 2、5 条与“不得虚构引文”复查。', revision: '补写“端”是萌芽、开头之意，并用孺子将入井的通行例子说明；补出回应之后仍须交代的条件；“倒灌”改为平实说法；荀子那句改为转述、去掉引号，不再以原文形式呈现；“不能用性恶推成”改为“推出”。' },
  ...
  'pt-buddhist': {
    ...
    objections: [
      { kind: '概括', text: '无我面对一个真正困难：若没有不变主体，记忆归属、业报和修行进展凭什么连接？印度佛教文本给出的不是一个答案，而是一组互相竞争的方案：有的用心识刹那之间的因果关系，说明前后如何相接；有的用“有分心”——即一段生命过程底下持续运作的那一层心识——说明这段过程为什么算作一个连续体；有的说心识之流会“摄取”并携带某些印记（“种子”），业由此传到后来；有的进一步设定藏识作为更强的载体；还有的用还原与随附来安置“人”的地位——随附的意思是：底下那些身心过程若一点差别也没有，“人”这一层就不会有差别，因而不必在它们之外另设一个东西。这些方案彼此竞争，没有哪一个被各派共同接受。', sourceIds: ['BUD-2'] },
    ...
    review: { mode: '自审（尚未独立复审）', checkedOn: '2026-09-11', findings: [
      ...
      { location: '内部论辩与解释争议｜连续性诸方案与空性；文本层次与概念边界', issue: '“随附”“有分心”“摄取”“种子”只有名称就被当作可比较的方案并列，读者无从判断它们各自在主张什么；空性一段的“约定层次”“二谛”同样未解释即被用来承担回应。', evidence: 'BUD-2 §5.1–§5.6 逐节给出这些方案，BUD-3 §2 界定空性与自性。按内容整改标准第 2 条（不得用未解释的术语解释术语）复查。', revision: '逐一补出各方案在说什么，随附写成“底层若无差别则上层无差别”；空性一段写出二谛之分及它所回应的问题，并说明回应到此并未结束；另在“文本层次与概念边界”注明无我的这一表述是各派共同针对的对象，不是某一派的完整主张。' },
  ...
  'pt-nyaya': {
    ...
    origin: [
      ...
      { kind: '概括', text: '印度认识论并无单一清单：正理与其他学派对有效认识手段的数量、证言地位、对象和错误理论有不同答案。把 pramāṇa 简单译成“证据”会漏掉它的技术含义：它指的不是摆在人面前的材料，而是使一次认知成为知识的那种来源或过程——知觉、推理、证言各算一种。它问的是“这次认识是怎样得来的、这条路可不可靠”，而不是“手上有哪些材料”。', sourceIds: ['NYA-1'] },
    ],
    boundaries: [
    ...
    review: { mode: '自审（尚未独立复审）', checkedOn: '2026-09-11', findings: [
      ...
      { location: '认识手段与推理边界｜遍在关系；历史起点与论辩任务｜pramāṇa', issue: '“推理需要被推论项与理由项之间的遍在／伴随关系”一句把三个未解释的术语堆在一起，正是用术语解释术语；“它说的是‘把认知造成为知识的那种来源’”是生硬直译。', evidence: 'NYA-1 §1.1 把 pramāṇa 定义为以正确方式造成认知的知识来源，§5 专论推理；NYA-3 §1.b.ii 给出五支证式。按内容整改标准第 2、5 条复查。', revision: '用烟与火先把两个位置分开，再写出遍在关系是什么；pramāṇa 一句改为可读中文并点明它问的是来路而非材料；“定说之定义”补出它指伽格舍自己认为立得住的那一个；另把“五支不是形式逻辑的失败版本”一条写出实际差别。' },
  ...
  'pt-being-change': {
    ...
    boundaries: [
      { kind: '概括', text: '数值同一性问的是：前后是不是同一个对象，而不是两个相似的对象；质的相同或相似则问两个对象共享哪些性质。这个定义本身不回答一个对象在改变性质、更换零件之后是否仍然存在——那要另外给出持续条件，不能直接从定义得到答案。把某一项持续条件（例如「必须共享全部部分」）写成数值同一性的通用要求，就是把一派的主张当成了词义。', sourceIds: ['BEC-1'] },
  ...
  'pt-knowledge-sources': {
    ...
    confusions: [
      ...
      { kind: '解释性重构', text: '“相信专家”不等于放弃批判。可以追问的地方很具体：他能不能把理由讲清楚、有没有独立的核查、这个专业共同体内部怎样发现和纠正错误、以及他在这件事上有没有利益牵连。' },
    ],
    historicalContext: [
    ...
    review: { mode: '自审（尚未独立复审）', checkedOn: '2026-09-11', findings: [
      ...
      { location: '有力反对及回应｜证言是否可还原；容易混淆的地方｜怀疑论', issue: '证言那条答非所问：反对问的是“为什么不把证言都还原为个人推理”，正文却转去说印度传统要求说明什么，原反对一句未答。此外“要挡住它”属《存在与变化》已改掉的那类生硬说法，闭合原则也只有名称没有内容；“定义与边界”还先行判定“不必要求每个人亲自重做证据”，等于把一方的结论写进了定义。', evidence: 'KNO-1 的定位含 §5.5（证言），还原论与反还原论之争正在该节；§6.2 是对闭合论证的各种回应。按内容整改标准第 3、2、1 条复查。', revision: '写出还原论的主张与反还原论的代价论证，再接上正理一路继续追问的问题，该段 sourceIds 由 KNO-2 扩为 KNO-1、KNO-2；“挡住”改为“回应”并写出闭合原则的内容与怀疑论怎样用它推进；“定义与边界”里那句改为中立表述，把争点留给下一节。' },
  ...
  'pt-religion-reason': {
    ...
    boundaries: [
      {
        kind: '概括',
        text:
          '“恶的问题”至少有两种：逻辑版本试图证明某类神的属性与任何恶不相容；证据（归纳）版本则认为特定、严重或看似无意义的苦难降低该神存在的合理性。两种版本所需的回应强度不同：要回应前者，只需给出一个逻辑上可能、又大体可信的故事，说明某类恶与这样的神并非不能同时成立（这就叫“辩护”）；要回应后者，却得在证据层面处理苦难的规模与分布，说明这些苦难为什么没有把该神存在的合理性压低。',
        sourceIds: ['REL-2'],
      },
      {
        kind: '概括',
    ...
    review: {
      ...
      findings: [
        ...
        { location: '定义与边界｜两种恶的问题、先验与后验；问题起点｜三问', issue: '“挡下前者／挡下后者”“先验论证只能……被拦下”属《存在与变化》已改掉的那类生硬说法；“先验”“后验”“分析的”三个词在被用来判定反驳着力点之前都没有解释；问题起点第三段先列三问，随后却说“后三者彼此相关”，指代落空。', evidence: 'REL-1 §2 是先验论证、§3 是后验论证，本页正是据这条顶层分法判断反驳该往哪里使力；REL-2 §4 给出辩护与神义论的举证责任差别。按内容整改标准第 2、5 条复查。', revision: '“挡下／拦下”改为“回应／质疑”；就地解释先验、后验与“分析的”；“后三者”改为“这三问”，三问各自补写完整。' },
      ],
  ...
  'pt-science-reality': {
    ...
    confusions: [
      ...
      { kind: '解释性重构', text: '可证伪性——要求一个理论能说出什么样的观察结果会与它冲突——是区分科学与非科学的重要思路之一，但它定不下全部科学实践，也不等于一次预测落空就必须放弃整个理论：落空的可能是某个辅助假设，也可能是测量本身出了问题。' },
    ...
    review: { mode: '自审（尚未独立复审）', checkedOn: '2026-09-11', findings: [
      ...
      { location: '历史与实践取向一段；定义与边界｜“结构”指什么', issue: '“自然事实由共同体投票决定”正是《存在与变化》已经改掉的那类生硬比喻；“积累的是形式或结构”没有说明这里的“结构”指什么；“未被设想的替代理论”也只给了名称。', evidence: '内容整改标准第 5 条列出的同类表达清单含“投票”；SCI-4 §1–§2 以菲涅耳的方程为例说明保留下来的是形式或结构，SCI-1 §3.1 是欠决定与未被设想的替代方案。', revision: '“投票”改为“由共同体的多数意见定下”；就地说明“结构”指理论用方程写下的那些关系；补出“未被设想的替代理论”指当时无人想到、因而从未被拿来比较的理论；另把可证伪性一条补上“落空的也可能是辅助假设”。' },
  ...
  'pt-environment-animals': {
    ...
    boundaries: [
      { kind: '概括', text: '说一个存在者“有道德地位”，是说它本身就构成我们行动时必须计入的一项理由，而不只是因为伤害它会连累到别人。这一句并不自动给出具体做法，也定不下它在冲突中的优先级；接下来还要问：它有哪些利益、是否享有权利、与我们处在什么关系中，以及利益相撞时怎样权衡。', sourceIds: ['ENV-2'] },
    ...
    review: { mode: '自审（尚未独立复审）', checkedOn: '2026-09-11', findings: [
      ...
      { location: '问题起点｜辛格；定义与边界｜道德地位', issue: '“辛格以平等考虑利益为起点批评仅按物种归属忽视痛苦”一句主谓宾纠缠，读不出他主张什么，也没有引出“物种歧视”这个后文要用的词；“有道德地位”被写成“能提出应被考虑的道德要求”，用在物种与生态系统上并不通顺。', evidence: 'ENV-3 文首明说平等要求的是平等的考虑而非同等待遇，并循边沁以受苦的能力为判准；ENV-2 §1 给出道德地位的界定。按内容整改标准第 5、1 条复查。', revision: '把辛格的原则分句写出、点明“平等的考虑不是同等的待遇”，并就地引出“物种歧视”；道德地位改写为“它本身构成我们行动时必须计入的一项理由”，后续要问的四件事保留；另把“内在价值”一条补足。' },
  ...
  'pt-identity-oppression': {
    ...
    objections: [
      ...
      { kind: '争议性判断', text: '来自框架外部的反对是另一回事，而它们更难答。第一条针对推论本身：观察到某个群体在结果上系统性地更差，还推不出这里存在结构性不义——差异也可能来自不同的偏好分布、地域与产业结构，或来自已经终止的历史原因。要从差异走到不义，必须指出是哪条规则、哪种分工、哪套程序在此刻仍持续产生这个结果。这条反对不否认结构性不义存在，它否认的是从统计差异直接读出不义；接受它的代价是每一项主张都要背上机制说明的举证责任。' },
      { kind: '概括', text: '第二条针对责任的承担者。如果只有个体才作道德选择，那么“结构”“制度”“群体”就不该是被谴责的对象。这条反对有它自己的经典表述：价值属于个体，唯有个体是道德责任的承担者，除非某人自己认为某行为是错的，否则他没有道德过错；为一个人并未打算造成的结果责备他是不公平的；集合体不作道德选择，因而不能被恰当地归以道德责任。要回应它，就得说明集体责任或结构性责任究竟怎样落到具体的人、岗位与决策上，而不是停在“制度有责任”这一句。值得注意的是，这场争论的现有材料主要围绕有组织的群体行动与集体意图展开，对“无人意图的累积后果”本身谈得很少——也就是说，结构性不义要的那种责任概念，恰好落在这场争论覆盖较弱的地带。', sourceIds: ['ID-6'] },
    ...
    review: { mode: '自审（尚未独立复审）', checkedOn: '2026-09-11', findings: [
      ...
      { location: '定义与边界｜立场理论；有力反对及回应｜集体责任一段', issue: '“立场理论”在反对一节被当作已知名词使用，而“定义与边界”只描述了它的结论、没有写出它主张什么；同段“也不是可以被多数轻易取消的证言”“何时具有优势”主语不明；集体责任一段用了“要挡住它”这一生硬说法，另一处外部反对还写作“还不蕴含存在结构性不义”。', evidence: 'ID-3 §2.1 是立场理论及其内部争论（含内化压迫与“一个女性主义立场”是否连贯）的来源定位。按内容整改标准第 2、5 条复查。', revision: '在“定义与边界”先写出立场理论的主张，再分两面写它必须面对的内化压迫与它不该被轻易取消的理由，并补足主语；“挡住”改为“回应”，“蕴含”改为“推不出”；“身份可以是……”一条也改写为可读的列举。' },
  ...
  'pt-mind-self': {
    ...
    boundaries: [
      ...
      { kind: '概括', text: '“同一论”“还原论”“功能主义”不是三个语气不同的同义词，读到它们时要能分开。同一论（心脑同一论）主张心理状态与过程就是脑的状态与过程，不只是与之相关；它有两种强度——类型同一论把某一类心理状态整体等同于某一类脑状态，个例同一论只主张“这一次的这个疼痛”与“这一次的这个脑过程”是同一件事。还原论一般指把某一层次的现象最终由更基础的层次说清、从而不必再当作独立的解释单位；类型同一论是它在心灵问题上的一个版本。功能主义换了一个提法：一个状态之所以是思想、欲望或疼痛，不在于它内部由什么材料构成，而只在于它在所属的认知系统里起什么作用——也就是它与感官刺激、其他心理状态和行为之间处在什么样的因果关系中。既然作用才算数，同一种功能状态就可以由很不相同的东西来担当：碳基的脑可以，硅基的装置原则上也可以（这一点通常叫“可多重实现”）。因此功能主义与类型同一论冲突——后者要把某一类心理状态整个等同于某一类脑状态——却可以与个例同一论并存：每一次具体的疼痛仍然就是某一段具体的物理过程。', sourceIds: ['MS-4', 'MS-5'] },
  ...
  'pt-ai-future': {
    ...
    boundaries: [
      ...
      { kind: '概括', text: '这里要分开两个角色。道德患者（moral patient）指承受道德对待的一方：说某个存在者是道德患者，就是说我们对它本身负有直接义务，而不只是因为伤害它会连累到别人。道德行动者则是能承担义务的一方，要满足更强的条件——知道自己在做什么、对行为有控制、能按理由行动。一个存在者可以是道德患者而不是道德行动者，婴儿和动物是常被举出的例子。法律人格、因果上的影响力与道德责任同样要分开：一个系统在因果上左右了结果，不等于它就是可被责备的那一方；法律是否给某个实体人格地位，也另有自己的考虑。', sourceIds: ['AI-4'] },
    ],
    objections: [
