...
export const coreEntryLedgers: Record<string, CoreEntryLedger> = {
  ...
  'pt-freedom': {
    ...
    origin: [
      ...
      { kind: '解释性重构', text: '把问题分成三问——「他本可以做别的吗」「这个行动是不是出自他自己」「应不应当赞许、谴责或要求补救」——可以避免把形而上学的结论直接当成政策结论。即使不承认基本应得（即：一个人仅仅因为做了那件事就理应受到惩罚或赞许，不必再诉诸预防犯罪之类的别的好处），预防、保护、说明与补偿仍然要安排。' },
    ],
    boundaries: [
      { kind: '概括', text: '相容论与不相容论的分歧不在「人有没有在选择的感觉」，而在道德责任所要求的那种行动控制究竟要求什么：相容论认为这种控制与决定论可以并存，不相容论认为真正要紧的那种自由不可能与它并存。两种常见的替换会让这个争点提前消失——把决定论换成宿命论，争的就成了「做什么都没用」；把它换成「原因已经被科学完全发现」，争的又成了一个经验问题。这两问都不是本页要处理的那一问。', sourceIds: ['FRE-1', 'FRE-2'] },
      { kind: '概括', text: '「道德责任」也不是一个问题，而要分成可归属性、可要求说明、可追究性三问；三者各问什么，本页的概念卡逐条写出。这里要接着说的是它们为什么必须分开：三者许可的反应不同，所需的控制条件也未必相同——一个行为确实出自某人的品格，他是否应被追究仍要另行判断。看到一次错误就直接跳到「此人应受惩罚」，等于把这三问一次跨过。', sourceIds: ['FRE-2'] },
    ],
    objections: [
      ...
      { kind: '概括', text: '若为摆脱决定论而把行动归于随机事件，随机性不会自动给行动者更多控制。运气论证与「回卷」论证的着力点正在这里。所谓回卷，是设想把时间倒回作选择之前的同一刻反复重放：如果选择是非决定的，那么在完全相同的历史之下，这一次这样做、下一次那样做，而这个差别不由行动者的任何理由、意图或品格决定。两个论证由此主张：非决定的选择要么使行动者失去控制，要么无法被充分解释。自由意志论（主张自由必须以非决定论为条件的那一派）因此还得说明：一个非决定的行动，凭什么仍然可以恰当地把行动者算作它的源头。', sourceIds: ['FRE-1'] },
    ],
    confusions: [
      { kind: '解释性重构', text: '解释一个行为的成因，不等于为它开脱；但它可能改变我们该作何回应——是治疗、约束、要求补救，还是谴责。' },
      { kind: '解释性重构', text: '没有外在胁迫只是相容论要用到的条件之一，不等于「这个行动为什么属于我」这个问题已经解决。' },
  ...
  'pt-logic': {
    ...
    origin: [
      { kind: '概括', text: '人们不仅会争论结论，也会争论结论是否由给出的理由支持。逻辑把这种支持的结构显明：在经典演绎中，有效意味着：无论把句子中的词项怎样赋义，都不会出现前提全真而结论为假的情形。这只评价形式是否保住真，并不保证前提本身为真。', sourceIds: ['LOG-1'] },
      { kind: '概括', text: '日常论证还要处理含混语境、概率、因果和对话目标。非形式谬误研究因此不只把句子符号化，而要说明一种论证为何看似有力、在哪个语境和标准下失败。', sourceIds: ['LOG-2'] },
    ],
    boundaries: [
      { kind: '概括', text: '有效与健全各自说的是什么，本页的概念卡已经写出；这里要接着分开的是另外两个常被当成同一件事的词。可推导性问的是：在一套给定的推演规则里，能不能一步步把结论推出来。语义有效性问的是：在任何一种为语言中各词项指派意义的方式（这样一种指派就叫一个「解释」或「模型」）之下，只要前提全真，结论是否都不可能假。前者是按规则一步步走，后者是把所有可能的赋义一并考虑。两者是否总是重合——推得出来的是不是都有效、有效的是不是都推得出来——属于元逻辑问题，也就是拿这套逻辑系统本身当研究对象的问题。', sourceIds: ['LOG-1'] },
      { kind: '解释性重构', text: '归纳和最佳解释推断不承诺结论必然为真，而比较样本、替代假设、反例、预测和背景知识给予的支持力度。它们不能由一张「形式有效／无效」表格代替。' },
    ],
    objections: [
      { kind: '概括', text: '把经典逻辑视为唯一正确逻辑会忽略直觉主义、相关逻辑等针对不同语言、推理目标或悖论的方案；反过来，存在多种逻辑也不意味着规则可以随意挑。改用哪一套，要说明清楚四件事：处理的是哪一种语言、给它配上什么语义、要完成的是什么推理任务、以及这样换要付出什么代价。', sourceIds: ['LOG-1'] },
      { kind: '概括', text: '「谬误」这个标签没有一套适用于每种情形的机械定义：同一段推理算不算谬误，会随歧义、所处语境和这场对话要达成什么而变。所以这一层要做的不是给论证挑一个名目，而是把它的前提、结论和隐含跳步重建出来，再指出它在这个语境下为什么失败。', sourceIds: ['LOG-2'] },
      { kind: '概括', text: '归纳还有一个比「支持强弱」更硬的问题。休谟的两难是：为归纳作辩护的论证只能是论证性的或概率性的。论证性推理无法从「过去如此」必然推出「将来如此」，因为反面并不自相矛盾；概率性推理则要预设自然齐一，而这条预设本身只能由归纳支持，于是循环。后续回应分别攻击两支：有的主张齐一性或某种解释原则可作先天／可辩护的前提，有的接受规则循环式的自我支持，有的干脆改换「正当」的标准，用长程收敛或形式学习理论的最优性来交代。哪一支成功仍是未决问题——把归纳一句带过、只说它「靠反复检验就够了」的讲法，漏掉的恰恰是这里。', sourceIds: ['LOG-6'] },
    ],
    confusions: [
      { kind: '解释性重构', text: '前提假而论证有效并不矛盾；它只说明若前提为真，结论不能为假。' },
      { kind: '解释性重构', text: '相关性不是因果性，统计关联也不自动消除混杂、选择偏差和替代解释。' },
      { kind: '概括', text: '但「相关不等于因果」也不推出「只有实验才能研究因果、观察数据没有帮助」。真正推不出因果的，是单凭统计关联这一条路：任何因果结论都要依赖某个本身就带因果内容的前提——例如哪些变量可能影响哪些变量、有没有同时影响两端的共同原因。一旦把这些前提写明，并说明所要估计的那个因果量在这套前提下确实能由手上的数据算出来，观察数据同样可以给出不系统偏向某一侧的估计。缺的始终是可辩护的因果假设，不是样本量；反过来，实验也不是无条件的保证。', sourceIds: ['LOG-8'] },
      { kind: '解释性重构', text: '形式化既是澄清工具，也是一种模型：它会略去语气、语境、权力关系和事实调查。用它的时候要交代清楚，这一次形式化保留了原论证的哪些部分，又把哪些部分留在了模型外面。' },
    ],
    historicalContext: [
  ...
  'pt-language-meaning': {
    ...
    origin: [
      ...
      { kind: '概括', text: '早期中国的名（ming）与实（shi）讨论将正确用名连接到行动、礼、秩序与论辩。它不是现代语言学、也不是当代指称理论（研究词怎样指向对象的理论）的原样前身，却提供了另一套问题框架：在那里，语言从一开始就带着规范作用和实践后果。', sourceIds: ['LAN-3'] },
    ],
    boundaries: [
      { kind: '概括', text: '词的意义、句子的真值条件、说话者意图和话语效果是四件不同的事。说清「冰箱里有牛奶」在什么情况下为真，和说清某人这次说它是想达成什么、实际又造成了什么，并不是同一项工作。一个词可以有稳定的用法，却在不同语境中指向不同对象；一句在事实上没说错的话，也可能在特定场合造成误导。', sourceIds: ['LAN-1', 'LAN-2'] },
      { kind: '解释性重构', text: '「真理承担者」这个词要先说清：它问的是「真」这个评价到底加在什么东西上——是一个信念，一个命题（即句子所表达的内容），一个句子，还是某人某一次说出的那段话。分开它们有实际用处：「我明天来」这同一个句子，由不同的人在不同的日子说出，真假各不相同；而它每一次所表达的内容一旦定下，真假就不再随谁来说而变。' },
      { kind: '概括', text: '选定哪一种承担者会牵动后面的理论怎么写：主张真在于与事实相符的一路（对应论），和把真直接系于句子真值条件的一路，对承担者的要求并不相同。语言与真理关系紧密，却不能把一切有意义的言语都缩成在断言事实——请求、承诺与判决各有自己的成败条件，用真假去评价它们并不合适。', sourceIds: ['LAN-2'] },
    ],
    objections: [
      { kind: '概括', text: '一条路线把意义主要交给指称。它的难处在于：「孙悟空」没有对应的对象，「三」不是可以指给人看的东西，「时间是一条河」并不是在报告某条河，而「不该说谎」这类规范性语言（即说什么该做、什么算对错好坏的话）也不像是在指认什么。另一条路线把意义主要交给使用：一个词的意思就是它在实践中被怎样用。它要回答的则是另一组问题：既然对错由用法决定，那么「你这个词用错了」「这种说法本身有问题」凭什么还能成立，真理又为什么不只是某个群体碰巧形成的习惯。比较两条路线时该问的是：各自把哪些现象解释掉了，又把哪些现象留给了别人。', sourceIds: ['LAN-1', 'LAN-2'] },
      { kind: '概括', text: '有一种批评说：名实、正名讲的不过是由上位者规定词该怎么用。这个批评有力，却下得太快。它漏掉了文本里的另一层：用名是否正确，在这些文本中关系到一件事能不能做成、一套角色关系能不能运转，而不只是词典条目对不对；它也漏掉了「谁有资格定名」这一问在这些文本内部同样可以被追问。反过来说，指出这些规范有其历史语境，并不等于它们从此免受批评。', sourceIds: ['LAN-3'] },
    ...
    review: { mode: '自审（尚未独立复审）', checkedOn: '2026-09-11', findings: [
      ...
      { location: '主要立场｜「解释与权力敏感取向」的来源账', issue: '该立场（命名与分类会分配可见性、资格、资源与责任）此前唯一的核对来源是 LAN-3，而 LAN-3 是早期中国的名实与逻辑条目，其定位「§2 孔子与名的秩序」不涉及当代关于分类、制度权力与认识不公的论证。', evidence: '本轮实际打开 LAN-3 全文：全部章节为背景、孔子、墨家、道家、荀子、结语，无任何当代分类政治或认识不公内容。', revision: '新增 LAN-7（SEP：Feminist Social Epistemology §4.1、§2.1），并在「有力反对及回应」写出证言不公与解释不公的机制，同时写明推不出什么。这个立场的名字仍是自造的「取向」式说法，应改为它实际所属的理论名（认识不公／社会构成论）；立场的命名不在这一层，须另行处理。' },
    ...
    ], remaining: ['尚未逐节核验《论语》《墨经》《荀子·正名》的中文校勘版本。', 'Haslanger 一路关于分类的社会构成论证尚未建立来源账；LAN-7 只覆盖认识不公一侧，不覆盖「分类如何因果地塑造人类种类」。'], adjacentImpact: '「解释」页须区分文本意义与作者意图；「身份、压迫与认识不公」页应说明命名的制度后果而非只作修辞批评，它与本条引用的是同一份综述的不同章节。本页的历史语境一栏只列传统与时段线索；与「什么是好推理与有效论证」这类核心问题页的连接放在关系层，那里要求每条连接说出自己的理由。', nextPriority: '补写言语行为、翻译和中国名实论辩的原典阅读路径，并为社会构成论建立独立来源账。' },
  },
  'pt-western-modern': {
    status: '核验正文｜自审完成',
    scope: '17—18 世纪的欧洲哲学在这里按四条线索铺开：知识、自然、心灵、政治权威。要提防的读法是把它读成「理性主义战胜经验主义」的单线进步史；这条叙事的来历和它错在哪里，本页留到「史学分类的争议」一节处理。同一时期中国、印度、伊斯兰世界的近世哲学各有自己的分期，这一页代表不了它们。',
    ...
    boundaries: [
      { kind: '概括', text: '「理性主义／经验主义」这对标签原本的适用范围有限：它只就知识的来源发问——一项知识是靠感官经验取得，还是不依赖具体经验就能取得。「先天」在这里说的就是后者：不依赖具体感觉经验而成立。围绕这对标签的三个论题（直觉／演绎、先天知识、先天概念）在提出时都写成「在某一类主题上」的主张，所以同一个哲学家完全可以在数学上偏重理性、在关于物体的知识上偏重经验；把整个人贴上一个标签，反而看不清他在哪一类问题上主张了什么。这两个标签也没有穷尽知识的来源——诉诸启示或某种洞见的主张就不在这两格之内。', sourceIds: ['MOD-1'] },
    ...
    objections: [
      { kind: '概括', text: '「大陆理性主义对英国经验主义」的讲法易于教学，却会误导：这条叙事把笛卡尔、斯宾诺莎、莱布尼茨排在前，把洛克、休谟、里德讲成一场逐步抛弃前者形而上学主张的接力；而相关综述明确指出，这样排本身就是一种误读。回应不是取消比较，而是每次都说明这一次比较的是哪个领域、到哪里为止。', sourceIds: ['MOD-1', 'MOD-2'] },
  ...
  'pt-confucian': {
    ...
    boundaries: [
      { kind: '概括', text: 'Ru（儒）早于孔子而与礼乐、经典专家相关；“Confucian”既可指早期文本，也可指后来的哲学、教育和制度传统。阅读时要随时标明自己在说哪一层，不要把后来两千年的制度实践反过来读进《论语》。', sourceIds: ['CON-1'] },
    ...
    objections: [
      { kind: '概括', text: '孟子说的「端」是萌芽、开头的意思：像看见孩童将要跌入井中时的那一阵不忍，是善的起点，不是已经完成的善。针对这条思路的质问是：既然人人都有这样的开端，恶行、贫困、暴力和偏私又是从哪里进入这套理论的？回应是把端绪说成一种可以扩充、也可以被遮蔽的能力——它要养，不养就长不成德性。但这个回应要站得住，还得接着交代：教育、家庭与物质条件具体是怎样支持或损坏这种养成的。', sourceIds: ['CON-2'] },
      { kind: '概括', text: '荀子把自然欲望、有意的努力（wei）、师法与礼联系起来，能解释为何德性需要学习：冲动本身并不可鄙，问题出在不经反省的发作会造成伤害，礼与乐提供了可用的出口。批评者则问这种秩序如何避免把既有权威定为标准——而「道」究竟是圣人发现的、还是人所构造的，正是荀子研究里尚无定论的争议：有一段文本说「道」不是天之道、也不是地之道，而是人所行的道；另一些段落又要人观察四时运行的规律。', sourceIds: ['CON-3', 'CON-4'] },
    ],
    confusions: [
      ...
      { kind: '解释性重构', text: '搬出孝与礼，不等于关系中的不平等、伤害与异议就自动被压下去了；坏的角色期待该怎样批评，在这些文本内部仍是有待论证的问题。' },
    ...
    ], remaining: ['尚未逐段核对《论语》《孟子》《荀子》的中文校勘本与早期注本。', '「礼可能被用来固化不平等」这一判断，目前只由 CON-3 §2.10（儒家与性别）作为一个独立争论题目支持，未核对该节内部各方论证。'], adjacentImpact: '「关系、照护与依赖」「因果、自由与责任」「何种权力与制度是正当的」三页可在此互链，但不能把儒家的关系性直接当作当代规范结论。本页的历史语境一栏只列同时期的传统线索；与这些核心问题页的连接放在关系层，那里要求每条连接说出自己的理由。', nextPriority: '补写墨家、道家、法家与宋明重构，以呈现儒家之外和儒家内部的连续论辩。' },
  },
  'pt-buddhist': {
    status: '核验正文｜自审完成',
    ...
    origin: [
      ...
      { kind: '概括', text: '后来的论师并未只重复同一套口号：部派阿毗达磨、以龙树为代表的中观、瑜伽行及佛教逻辑认识论，分别就法、空性、心识、推理与语言发展争论。说「佛教认为」如何如何，等于把这几派互不相让的结论并成一个，谁在反对谁就看不见了。', sourceIds: ['BUD-2', 'BUD-3'] },
    ],
    boundaries: [
      { kind: '概括', text: '无我所针对的，是被执取为恒常、独立、能作主宰的那个自我；它不取消日常的人称、记忆、因果连续，也不取消伦理训练。各派对这一点的具体说法并不一致，这里给出的是它们共同针对的对象；至于人格、心识与业各自怎样安置，不同传统的解释相差很大。', sourceIds: ['BUD-1', 'BUD-2'] },
      { kind: '概括', text: '中观的空性以缘起反对自性：事物不是靠自身、脱离条件而成立。它需要与二谛、论辩对象和后世注释传统一起理解，不能被换成近代哲学里的某一种反实在论。', sourceIds: ['BUD-3'] },
    ],
    objections: [
      { kind: '概括', text: '无我面对一个真正困难：若没有不变主体，记忆归属、业报和修行进展凭什么连接？印度佛教文本给出的不是一个答案，而是一组互相竞争的方案：有的用心识刹那之间的因果关系，说明前后如何相接；有的用「有分心」——即一段生命过程底下持续运作的那一层心识——说明这段过程为什么算作一个连续体；有的说心识之流会「摄取」并携带某些印记（「种子」），业由此传到后来；有的进一步设定藏识作为更强的载体；还有的用还原与随附来安置「人」的地位——随附的意思是：底下那些身心过程若一点差别也没有，「人」这一层就不会有差别，因而不必在它们之外另设一个东西。这些方案至今没有哪一个被各派共同接受。', sourceIds: ['BUD-2'] },
      { kind: '概括', text: '空性也被批评为会削弱因果与规范：事物若都没有自性，因果、善恶与修行的差别还立得住吗？中观的回应要用到二谛之分——世俗谛是日常语言、因果关系与修行实践照常成立的那一层，胜义谛是就「事物有没有自性」所作的分析。按这个分法，被否定的是自性，不是世俗层面的因果与规范。但回应到此并未结束：被称为「约定」的东西凭什么仍有约束力、两谛之间究竟是什么关系，至今仍是这一传统内部的解释争议。', sourceIds: ['BUD-3'] },
    ],
    confusions: [
      { kind: '解释性重构', text: '「无我」不等于否认痛苦、责任或他人，也不自动等于现代心理学的「自我只是幻觉」。' },
      { kind: '解释性重构', text: '「空」不等于「虚无」；「缘起」说的是事物依条件而生起，也不是说凡见到相关就算找到了原因。', sourceIds: ['BUD-3'] },
  ...
  'pt-nyaya': {
    ...
    origin: [
      ...
      { kind: '概括', text: '印度认识论并无单一清单：正理与其他学派对有效认识手段的数量、证言地位、对象和错误理论有不同答案。把 pramāṇa 译成「证据」会漏掉它的技术含义——它问的不是「手上有哪些材料」，而是「这次认识是怎样得来的、这条路可不可靠」；知觉、推理、证言各算一条这样的来路。', sourceIds: ['NYA-1'] },
    ],
    boundaries: [
      { kind: '概括', text: '从烟推知火不是「看到烟就猜有火」。这个推理里有两个位置要先分开：山上有火，是要推出来的那一项；山上有烟，是据以推出它的理由那一项。支撑这一步的，是两者之间的遍在关系（vyāpti）——凡有烟处必有火，也就是说，理由那一项出现的地方，要推出的那一项无一例外地也出现。正理传统接着要处理的是：这条普遍关系凭什么成立、有没有反例、在什么观察条件下才成立、以及这次推理是为什么而作。五支论证（命题、理由、例证、应用、结论）也不是给内心推理多加三步：按正理自己的说明，这种成型的证式是要在教学与论辩场合把内心走过的推理摊开来，供他人逐支检查。', sourceIds: ['NYA-1', 'NYA-3'] },
      { kind: '概括', text: '证言在正理传统中算一条独立的知识来源：它与知觉、推理并列，而不是必须先还原成其中之一才作数。这一条界定的是争论的位置——接下来要争的不是「该不该听别人说」，而是让一次转述成为知识的条件是什么。', sourceIds: ['NYA-1'] },
    ],
    objections: [
      ...
      { kind: '概括', text: '这个难题在新正理手上并没有被宣布解决，而是被逼到定义层面：伽格舍（Gaṅgeśa，本页人物表用的是这个转写）在《真知宝》里逐一否决了此前二十一种关于遍在关系的定义，才提出他自己认为立得住的那一个（siddhānta-lakṣaṇa，「定说之定义」）。这说明该传统把「反例排除」当作一个技术上仍未收口的问题，而不是一句方法论口号。', sourceIds: ['NYA-2'] },
      { kind: '概括', text: '不同学派会争论：证言是否独立于推理、语言能不能可靠地指到它所说的那个对象、知觉里是否已经带着概念。因而「正理承认证言」不是终点，而是要进一步追问可信性的规范来自何处。', sourceIds: ['NYA-1'] },
    ],
    confusions: [
      { kind: '解释性重构', text: '「五支论证」不是现代形式逻辑的一个失败版本：它要完成的任务本来就不同——在场的对手要能一支一支地查验，其中「例证」一支还要求举出真实的实例，而形式逻辑判断有效与否并不要求这些。', sourceIds: ['NYA-3'] },
      { kind: '解释性重构', text: '承认证言是一种认识来源，不等于放弃检验；问题只是换了位置：怎样判断说话者在这件事上可靠、听者有没有正确理解那句话、消息经过转述有没有走样、以及反驳该怎样提出。' },
  ...
  'pt-knowledge-sources': {
    ...
    boundaries: [
      { kind: '概括', text: '「有真信念」不等于「有知识」，而盖梯尔的第二个案例把这句话做成了一条可以逐步检查的机制。史密斯有强证据支持「琼斯有一辆福特车」，于是从它演绎出「要么琼斯有一辆福特车，要么布朗在巴塞罗那」。这种「要么……要么……」的句子（析取式）只要有一支为真，整句就为真；既然他有充分理由相信第一支，他相信整句也就同样有正当理由。可是琼斯开的其实是租来的车，而布朗恰好就在巴塞罗那。于是三条件同时成立——析取式为真、他相信它、他的相信有正当理由——他却不知道它。要点不在「运气」这个词，而在正当性的来路：它经演绎从一个有正当理由的假信念传到了一个恰好为真的结论，使结论为真的那件事与他的理由毫无关系。此后的理论正是在补这个缺口：可靠过程、反事实安全性、认知能力或社会实践。', sourceIds: ['KNO-1', 'KNO-3'] },
      { kind: '解释性重构', text: '[[testimony|证言]]不是「未经验证的传闻」的同义词。依赖专家、历史档案和他人报告是认识生活的常态。评价一次证言时要看的是：说话者在这件事上有没有相应的能力、有没有利益冲突、这条消息经过了哪些人之手、出错之后有没有办法纠正。至于「每个人是否还必须亲自把证据重做一遍」，那不是定义能定下的，而正是下面要争的问题。' },
    ],
    objections: [
      { kind: '概括', text: '经验主义若把一切知识压到感觉输入，会难以说明逻辑、数学、概念和证言的作用；理性主义若把可靠性主要交给某种不依赖经验就已具备的先天结构，又得解释这种结构凭什么能接上具体的世界。较成熟的理论通常承认多种来源，却要说明它们冲突时的权重。', sourceIds: ['KNO-1'] },
      { kind: '概括', text: '把证言列为一种独立的认识来源，会遇到一条正面的反对：还原论主张，听到一份报告只是拿到了一条证据，真正让你有理由相信它的，是你自己关于「这个人在这类事情上可不可靠」的归纳推理；所以证言并不独立，它归到推理里去了。反还原论一路的回答不是重申证言重要，而是指出这条要求的代价：一个人所知道的绝大部分事情——历史、地理、自己的出生日期——都没有、也不可能有这样一份独立的归纳记录；若严格按还原论办，这些统统不算知识。于是要么承认证言自成一种来源，要么把知识的范围缩到极小。争论并未就此收口：正理一路继续追问的是说话者的可靠性凭什么评定、听者对语句的理解在其中起什么作用，以及对象不在眼前时知识怎样成立——而不是给权威贴一个标签了事。', sourceIds: ['KNO-1', 'KNO-2'] },
    ],
    confusions: [
      { kind: '解释性重构', text: '怀疑论不是「什么都不知道」这样一句口头姿态。它要求你指出：手上的理由为什么不足以排除错误、幻觉或运气。而且它通常靠一条看起来无害的原则推进——闭合原则：如果你知道一件事，又知道这件事必然推出另一件事，那么你也就知道后一件。怀疑论者正是顺着这条原则，从你自认为知道的事一路推到一个你显然不知道的结论，再回过头来要你交代起点。要回应它，就必须指出这条链上是哪一步不成立，而不是重复宣称自己确实知道。', sourceIds: ['KNO-1'] },
      { kind: '解释性重构', text: '「证据」不是一个个孤立的数据点：一样东西之所以能算证据，靠的是测量方式、背景假设、推理步骤和可以追问的来源共同支撑。' },
      { kind: '解释性重构', text: '「相信专家」不等于放弃批判，只是批判的对象换了：外行不必自己重做实验，却可以追问他能不能把理由讲清楚，以及这个专业共同体靠什么发现并纠正自己的错误。' },
    ],
    historicalContext: [
    ...
    ], remaining: ['尚未完成中文 pramāṇa 译名和正理原典版本的对勘。', '做梦论证与缸中之脑的具体重构本轮未写进本账；论证的逐步骨架由本页的论证地图承担，本账只交代闭合原则这一步。若要在研究层细述，须另核 KNO-1 §6.3–§6.5。'], adjacentImpact: '与「科学如何解释世界」连接时，不能把科学证据降为单一感官经验；与「身份、压迫与认识不公」连接时，应另说明证言不公与一般证言理论的关系。', nextPriority: '补写正理历史线索的阅读路径，并校对中文原典选本。' },
  },
  'pt-religion-reason': {
    status: '核验正文｜自审完成',
    scope:
    ...
    origin: [
      ...
      },
      {
        kind: '概括',
        text:
          '恶之所以构成一个论证而不只是一种情绪，可以摆出来逐句看：一个全能者有能力阻止任何一件恶；一个全知者知道每一件恶正在发生；一个全善者不会毫无理由地容许恶——而世上确有恶，其中有些既严重又看不出对任何人有什么用处。这几句话没法轻轻松松地同时成立，压力就是从这里来的。至于这份压力究竟有多强、回应要强到什么程度才算够，留给下一节和本页的论证地图去分。',
        sourceIds: ['REL-2'],
      },
      {
        kind: '解释性重构',
        text:
          '把讨论拆成三问——「是否有理由相信」「这些理由能否说服并不接受该信念的人」「持这种信念能否正当地指导实践」——可以避免把私人承诺、公共论证与形而上学结论混作一件事。这三问彼此相关，却没有哪一问的答案能自动决定另外两问。',
      },
    ],
    boundaries: [
      {
        kind: '概括',
        text:
          '「恶的问题」至少有两种：逻辑版本试图证明某类神的属性与任何恶不相容；证据（归纳）版本则认为特定、严重或看似无意义的苦难降低该神存在的合理性。分成两种是有用的，因为它们所需的回应强度完全不同：回应前者只要说明某类恶与这样的神并非不能同时成立，回应后者却要在证据层面处理苦难的规模与分布，说明这些苦难为什么没有把该神存在的合理性压低。把只够回应前者的话拿去回应后者，是这场争论里最常见的错位。',
        sourceIds: ['REL-2'],
      },
      {
        kind: '概括',
        text:
          '自然神学内部的论证也不是同一种。一类从世界的某个特征出发：宇宙有起因、自然有秩序与精细调节、有人报告宗教经验——这类叫后验论证，「后验」指前提取自对世界的观察。本体论论证则被划在先验一侧：它自称只用分析的、先验而必然的前提——「先验」指不依赖这类观察，「分析的」指仅凭概念本身就能定下真假——因而不从世上的任何事实取材。这个区别决定了反驳该往哪里使力：后验论证可以被追问「这个特征还有没有别的解释」，先验论证则只能在概念与推论本身上被质疑。',
        sourceIds: ['REL-1'],
      },
      {
        kind: '争议性判断',
  ...
  'pt-death-meaning': {
    ...
    boundaries: [
      { kind: '概括', text: '伊壁鸠鲁的论证从一个前提出发：一切好与坏都要通过感觉才成其为好或坏。死亡是一切感觉的终止，所以它对死者无所谓好坏——我们在时死亡未至，死亡至时我们已不在。剥夺论不与这个前提正面相撞：它不说死者还在继续感到痛苦，而是改用比较——把此人实际的一生，与他若不死本可拥有的那段好日子相比，差额就是死亡造成的损失。', sourceIds: ['DEA-1', 'DEA-2'] },
      { kind: '争议性判断', text: '「意义」不该被拿来判定一个受疾病、贫困或压迫所限的人生比较不值得过。如果某种理论把「一个人能自由挑选并投入的事业」当作意义的唯一来源，它就必须交代：这些结构性限制在评价里算在哪一处。', sourceIds: ['DEA-3'] },
      { kind: '概括', text: '把「意义」当成已经确定存在、只在来源上有分歧的东西，会漏掉一个仍被认真辩护的立场：虚无主义。本页三条路线都预设了意义可以成立，只在它从哪里来上分歧，因此谁也没有正面回答虚无主义提出的那一问。它的当代形态也值得留意——多数版本并不依赖有神论或灵魂，反倒常从「意义要求某种我们没有的东西」一类前提出发。', sourceIds: ['DEA-3'] },
    ],
    objections: [
      ...
      { kind: '概括', text: '主观主义一路把意义交给当事人自己的投入与满足，难处是空洞甚至有害的投入也会照算不误；客观主义一路把意义系于某些确实值得投入的对象，难处是它可能不理会当事人自己认不认同。沃尔夫的混合观点因此两边各取一条：意义出现在「主观吸引与客观值得相遇」的地方。但这不等于问题解决了——「什么算值得」原封不动地留在那里，只是换了个位置。', sourceIds: ['DEA-3'] },
    ],
    confusions: [
  ...
  'pt-science-reality': {
    ...
    boundaries: [
      ...
      { kind: '概括', text: '实在论并非必然主张每一个理论词都逐字对应真实对象；回应理论更替的一类策略是只承诺成熟理论中保留下来的部分。要点在于说清保留下来的究竟是什么：结构实在论的经典例子是菲涅耳的光学以太被麦克斯韦的场取代，关于「世界上有什么东西」的说法完全换了，而菲涅耳那套数学关系被保留下来。按这条读法，跨理论更替中积累的是形式或结构——这里说的「结构」指理论用方程写下的那些关系，不是关于世界由哪些东西构成的说法。', sourceIds: ['SCI-1', 'SCI-4'] },
    ],
    objections: [
      { kind: '概括', text: '对无奇迹论证的反对不是否认科学成功，而是质疑「理论大体为真」是不是对这份成功的唯一最佳解释，以及是否存在当时未被设想的替代理论——即在那个年头没有人想到、因而从未被拿来与它比较的理论。实在论者则要求反对者说明预测与干预的成功如何不诉诸某种世界结构。', sourceIds: ['SCI-1'] },
      { kind: '概括', text: '库恩讨论「革命」时关注规则、范式和共同体实践的改变：一门科学的发展不是匀速的，而在常规期与革命期之间交替，常规科学的形态是解谜。这提示科学史不能只列发现。但从「科学实践有它自己的历史」推不出「自然事实由共同体的多数意见定下」——库恩自己给出的分界是：否认某个认知过程出自应用理性规则，并不等于说它是非理性的过程。', sourceIds: ['SCI-5'] },
    ...
    confusions: [
      ...
      { kind: '解释性重构', text: '预测得准不等于解释得清，两者可以分开。同一批定律既能从旗杆高度和日照角推出影长，也能从影长和角度反推旗杆高度；两次推导同样有效、同样可用于预测，但只有前一次像是解释。所以「这个模型预测很准」与「这个模型说清了因果」是两句话；而一个用来指导干预的模型还要再多一项要求：它得说对「如果我们动手改变某个变量，结果会跟着怎么变」。', sourceIds: ['SCI-2'] },
      { kind: '解释性重构', text: '可证伪性是区分科学与非科学的重要思路之一，但它定不下全部科学实践。这里要拦的是它最常被误用的一步：一次预测落空并不逼人放弃整个理论——落空的可能是某个辅助假设，也可能是测量本身出了问题。' },
      { kind: '解释性重构', text: '承认价值取向、仪器条件与制度安排会影响研究，并不等于否认证据可以纠错，也不等于否认世界会反过来限制哪些解释站得住。' },
    ],
    historicalContext: [
  ...
  'pt-environment-animals': {
    ...
    origin: [
      ...
      { kind: '原文入口', text: '辛格的起点是平等考虑利益：一个存在者只要能受苦，它的痛苦就应与别处同等的痛苦一样被计入；仅仅因为它属于另一个物种就不计，等于把界线划在了与痛苦无关的地方（这就是他所批评的物种歧视）。要注意，他说的是平等的考虑，不是同等的待遇。这是一条以个体的感受和利益为中心的路线，不等于完整的生态整体论。', sourceIds: ['ENV-3'] },
    ],
    boundaries: [
      { kind: '概括', text: '「有道德地位」给出的只是入场资格：它让一个存在者进入我们必须计入的理由之列，却不给出任何具体做法，也定不下它在冲突中的优先级。真正难的部分从承认之后才开始——这个存在者有哪些利益、是否享有权利、与我们处在什么关系中，以及利益相撞时怎样权衡。把入场资格直接读成优先权，是本页最常见的一次跳步。', sourceIds: ['ENV-2'] },
    ...
    confusions: [
      ...
      { kind: '解释性重构', text: '说某样东西有「内在价值」，不是说人类从来没有用过它；这个说法要反对的是另一件事——它的价值完全取决于对别人有什么用，一旦没用就什么也不剩。' },
  ...
  'pt-identity-oppression': {
    ...
    boundaries: [
      { kind: '解释性重构', text: '「身份」可以指自我认同、他人的分类、法律上的类别、共同的政治行动，也可以指一个人在历史中所处的位置；同一个词在这几种用法里指的是很不相同的机制。讨论压迫时因此要落到具体处：是哪条规则、哪种分工、谁的证言不被采信、哪些空间或资源的分配，造成了可以追踪的劣势。' },
      { kind: '概括', text: '立场理论主张：一个人所处的社会位置会影响他能看到什么，处在被支配位置的人因此可能对某些关系看得比别人清楚。这条主张有两面。一面是它必须面对内化压迫——例如有研究指出，许多因遭到侵害而自责的女性，内化的是关于自身责任的错误信念；可见位置本身并不担保看得准。另一面是它也不该被轻易取消：来自边缘的说法既不是自动无误，也不能因为持这种说法的人少就当作可以不听。这里还留着一个未解的技术难题：既然女性的处境各不相同，凭什么还能说存在「一个女性主义立场」。所以，位置在哪些问题上带来认识优势，只能在具体领域里一项项检验。', sourceIds: ['ID-3'] },
    ],
    objections: [
      ...
      { kind: '概括', text: '第二条针对责任的承担者。如果只有个体才作道德选择，那么「结构」「制度」「群体」就不该是被谴责的对象。这条反对有它自己的经典表述：价值属于个体，唯有个体是道德责任的承担者，除非某人自己认为某行为是错的，否则他没有道德过错；为一个人并未打算造成的结果责备他是不公平的；集合体不作道德选择，因而不能被恰当地归以道德责任。要回应它，就得说明集体责任或结构性责任究竟怎样落到具体的人、岗位与决策上，而不是停在「制度有责任」这一句。这场争论的现有材料却主要围绕有组织的群体行动与集体意图展开，对「无人意图的累积后果」谈得很少；结构性不义所需要的那种责任概念，恰好落在它覆盖最弱的地带。', sourceIds: ['ID-6'] },
    ...
    confusions: [
      { kind: '解释性重构', text: '证言不公不是「不同意某人」：一次有理由的反驳并不构成证言不公。成立的条件是可信度的折扣来自身份偏见，而不来自证据。', sourceIds: ['ID-3'] },
    ...
    review: { mode: '自审（尚未独立复审）', checkedOn: '2026-09-11', findings: [
      ...
      { location: '容易混淆的地方｜2026-09-15 复审', issue: '证言不公的定义在本页出现三次——问题起点一次、常见误读一次、概念卡还有一次；「要落到哪条规则、哪种分工」这句方法要求也在「定义与边界」和外部反对第一条里各写了一遍。', evidence: '三处对证言不公的措辞都是「因身份偏见给出低于证据所应得的可信度」；两处方法要求列的项目相同。', revision: '常见误读只留它独有的那一步——有理由的反驳不算证言不公，成立的条件是折扣来自身份偏见而非证据；外部反对那一条不再重列项目，改为要求指出哪一条机制此刻仍在运作。' },
    ], remaining: ['残障哲学和非殖民传统的原典尚未逐篇核验，不能用本条替代。', 'ID-2 仍是 pending：需要一份可按章定位的 Fricker《Epistemic Injustice》（纸本或有目录的电子版），才能把「原始区分」写回原典层；在此之前该书只作书目指针。', '外部反对中的「统计差异不蕴含结构性不义」与「可信度调整的统计辩护」本轮未能找到可核对的来源，正文按「争议性判断」标出、不挂任何来源。下一轮须为这两条各建一条真实来源（前者可从结构性不义与歧视理论一路找，后者涉及可信度调整的规范性争论）。', 'ID-1 的定位指向 1989 年那篇论文，而精读层把这条来源挂在〈边缘的地图〉（Mapping the Margins, 1991）那张原典卡上——这是两篇不同的文章。该错挂在精读层，不在这一层，须把两者拆开，或为 1991 年那篇另建来源。'], adjacentImpact: '与「知识从哪里来」「正义」互链时，应把认知伤害与资源／权利分配分别说明，再讨论交集。本条与「语言如何承载意义与真理」页引用的是同一份社会认识论综述的不同章节，两页对「证言不公」的定义须保持一致。新增的 ID-6 与「自由、责任与道德运气」页的责任三分相邻，两处不要各自另造一套「集体责任」的定义。', nextPriority: '建立残障正义与殖民性知识生产的独立历史条目及来源账，并为上述两条外部反对补来源。' },
  },
  'pt-mind-self': {
    status: '核验正文｜自审完成',
    ...
    boundaries: [
      { kind: '概括', text: '「人格同一性」名下其实是好几个只松散相连的问题，读到这个词时要先问它这次指哪一个：这个人跨时间还算不算同一个人（持续问题）、一个存在者要满足什么才算得上道德行动者、作为一个活的生物体前后是否连续、以及一个人为什么要特别在乎将来那个「自己」会怎样。这几问不必有同一个答案，把它们混作一谈，争论就会在不同问题之间来回滑动。', sourceIds: ['MS-2'] },
      { kind: '概括', text: '「同一论」「还原论」「功能主义」不是三个语气不同的同义词，读到它们时要能分开。同一论（心脑同一论）主张心理状态与过程就是脑的状态与过程，不只是与之相关。它有两种强度：类型同一论把某一类心理状态整体等同于某一类脑状态，个例同一论只主张「这一次的这个疼痛」与「这一次的这个脑过程」是同一件事。还原论则一般指把某一层次的现象最终由更基础的层次说清，从而不必再当作独立的解释单位；类型同一论是它在心灵问题上的一个版本。功能主义与类型同一论冲突，这一点本页的概念卡已经写出；这里要补的是另一半——它并不与个例同一论冲突。否认「某一类心理状态整体对应某一类脑状态」，并不等于否认「这一次的这个疼痛就是这一段具体的物理过程」。所以这三个名称不排成一条由强到弱的直线：接受功能主义的人，仍然可以在个例这一层上是同一论者。', sourceIds: ['MS-4', 'MS-5'] },
      { kind: '解释性重构', text: '神经对应证实心理活动与身体深度相关，却没有单独在上面三者之间作出选择；哲学问题在于何种解释算足够。' },
    ],
    objections: [
      { kind: '概括', text: '两边各有一道自己的难关。二元论主张心与物是两类不同的东西，它要说明这两类东西怎么会互相作用：一个不占空间、没有质量的心理事件，是怎样使手臂抬起来的。物理主义主张心理现象归根到底都是物理的，它要交代的是另一件事：即使把某种体验与某段神经活动的对应关系查得再细，为什么这段活动会伴随「感觉起来是这样」的体验，仍然没有被解释——从「查到对应」走到「解释了体验」，中间那一步要补出来。两边都不能把对方的难题重述一遍，就当作自己这边已经得证。', sourceIds: ['MS-1'] },
      { kind: '概括', text: '无我立场面对责任与再生等连续性难题；佛教理论在不同学派中对此有不同处理，不能用「佛教认为没有人」概括。', sourceIds: ['MS-3'] },
    ],
    confusions: [
      { kind: '解释性重构', text: '「没有不变实体自我」不等于「没有人、经验或责任」。' },
      { kind: '解释性重构', text: '心理连续性不等于记忆一字不漏地保留。记忆常被拿来解释同一性，却带着两个麻烦：一是人会有虚假记忆，二是这条路有循环的危险——能被「记起」的似乎只能是自己经历过的事，那么用记忆去界定「还是不是同一个人」，等于先把要解释的东西用上了。', sourceIds: ['MS-2'] },
      { kind: '解释性重构', text: '二元论、「心灵可以不依赖身体而存在」与各宗教的灵魂观三者有重叠，却不能互相替换：主张心与物是两类不同性质的人，未必主张心灵能脱离身体单独存在；灵魂观还牵着不朽、救赎一类另外的问题。', sourceIds: ['MS-1'] },
    ],
    historicalContext: [
  ...
  'pt-ai-future': {
    ...
    boundaries: [
      ...
      { kind: '概括', text: '这里要分开两个角色。综述里说的「道德患者」（moral patient）指承受道德对待的一方——我们对它本身负有直接义务；道德行动者则是能承担义务的一方，要满足更强的条件：知道自己在做什么、对行为有控制、能按理由行动。这两问为什么不对称，本页的概念卡已经写出。还要再加一层：法律人格、因果上的影响力与道德责任同样要分开。一个系统在因果上左右了结果，不等于它就是可被责备的那一方；法律是否给某个实体人格地位，也另有自己的考虑。', sourceIds: ['AI-4'] },
    ],
    objections: [
      ...
      { kind: '概括', text: '把当前系统称作「行动者」，可以只取一个很薄的意思：它是一个做事的东西，会造成或调节后果。这与「可被责备的道德行动者」不是一回事——后者还要求知道自己在做什么、能按理由行动。也正因为如此，部署责任不能因为系统复杂、经手的人多，就在机构内部摊薄到没有人承担。', sourceIds: ['AI-4', 'AI-5'] },
    ],
    confusions: [
      { kind: '解释性重构', text: '任务能力、语言理解、主观体验、道德地位、法律责任是五个不同问题；任一项的肯定都不自动推出其余四项。' },
      { kind: '解释性重构', text: '「拟人化」不是机器有心灵的证据。它首先是一个界面事实：界面越像人，使用者的信任、依赖和责任判断就越会跟着改变——而这与机器内部到底发生了什么是两回事。' },
      { kind: '解释性重构', text: '风险管理回答不了心灵哲学的问题。它处理的是另一件事：在还不确定的状态下，由谁负责记录、由谁负责监督、受影响的人从哪里申诉、以及谁有权停用这个系统。', sourceIds: ['AI-5'] },
    ],
    historicalContext: [
