...
export const comparativeQuestions: ComparativeQuestion[] = [
    ...
    framings: [
      {
        ...
        note:
          '相容论把「属于我」拆成可以逐项检查的心理结构，检查的位置在因果链条的最后一段。层级式的版本看行动出自的那个意志是不是行动者自己想要的：她不只是有一个想做某事的欲望（一阶），还对这个欲望本身持肯定态度（二阶——我希望自己被它推动去行动），两层对得上才算。理由响应式的版本不看这个，只问行动所出自的那套意志机制对理由敏不敏感：给它一个够分量的相反理由，它会不会改主意；因此它不要求当时真有另一种可能。两条路都不试图让行动摆脱因果。',
        sourceIds: ['AG-1'],
      },
      {
        ...
        note:
          '这一侧的关键不是「无原因」，而是源头：自由要求某些事确实是我们致使它发生的，而我们之所以去这样致使，又不是被别的什么东西、别的什么人促成的。这个要求本身会陷入一个两难——若一切被决定，源头总落在我之外；若引入非决定性，选择又可能只是随机而非受控。操纵与设计案例是这条路线的主要直观支撑，但同一份综述也指出这类论证有循环之嫌：它在前提里就已经假定了，因果一旦追溯到我控制不了的源头就会破坏自由，而这正是它要证明的。',
        sourceIds: ['AG-2'],
      },
      {
        tradition: '康德的自我立法',
        ...
        note:
          '康德不问「我的选择有没有原因」，而问一个意志在什么条件下算是自己在立法。判准落在法则的出处上：它所服从的法则是不是由它自己给出，行动者能不能算作这条法则的作者。于是「属于我自己」在这一栏成了一个关于应当如何的条件，而不是关于这个行动由什么原因造成的事实。',
        sourceIds: ['AG-3'],
      },
      {
        tradition: '儒家的修养与礼',
        ...
        note:
          '儒家追问的不是某个瞬间的选择是否自发，而是一个人的反应从哪里来、能不能被改造。重量因此落在礼与乐上：按综述的说法，礼不只标示什么有价值，还改造欲望、把道德倾向养成起来；《论语·阳货》那句「性相近也，习相远也」讲的正是人的差别出在「习」上。于是「属于我自己」在这一栏指向性情的来历——一个行动更像我自己的，是因为它出自被长期塑造过的性情，而不是因为它摆脱了塑造。',
        sourceIds: ['AG-4'],
      },
      {
        tradition: '佛教的业与相续',
        ...
        note:
          '佛教一侧先取消了提问的一个预设：行动背后不必有一个持续的所有者。按综述，承载业（指有意的行为以及它日后结出的果）的是行动本身，而不是一个转生的灵魂；所以可以有再生而没有一个从这一世搬到下一世的灵魂，前世之人与今世之人「既非同一亦非不同」。归属问题于是从「谁是这个行动的所有者」转成「哪些条件还在相续、哪些造作还在起作用」。',
        sourceIds: ['AG-5'],
      },
    ],
    caution:
      '五栏各自在回答不同的问题。相容论与源头性要求在争一个概念分析：使行动自由的条件是什么。康德争的是规范条件，这可以在因果问题悬置的情况下成立。儒家的核心问题是养成——怎样使一个人的反应变得可靠，而不是划出自由行动的边界。佛教连「行动有一个持续的所有者」都不接受，它要处理的是苦与解脱。把它们排成一排，最容易生出的正是几条对不上的等式：儒家没有主张「决定论与自由相容」，佛教也没有主张「一切被决定」——缘起讲的是条件相依，不是可预测的必然。',
    pending: [
      ...
      '未核对把这几支传统直接对接的比较研究；上面关于它们如何不同的判断是本库的解读，不是引自来源的结论。',
    ],
    sources: [
      {
        id: 'AG-1',
    ...
    framings: [
      {
        ...
        nodeId: 'pt-mind-self',
        caresAbout: ['持续的必要充分条件', '心理连续性与记忆', '裂变案例：一个人若分成两个，哪一个才是他', '动物论与纯物理连续性'],
        note:
          '这一支把问题定为持续问题：一个过去或未来的存在者要算作现在的某人，必须满足什么，满足了又是否就够。心理连续论的一个版本把条件定在记忆上——某个过去或未来的存在者就是你，只要你现在能记起她当时的某个经验，或者她能记起你现在的经验；动物论一侧不走心理这条路，主张我们就是有机体，持续靠纯物理的连续性。争的是数值同一（前后是不是同一个对象，而不是两个相似的对象）该由什么担保。',
        sourceIds: ['PI-1'],
      },
      {
        tradition: '佛教的无我分析',
        ...
        note:
          '佛教一侧不为这个「同一」找担保，而是拆掉提问的形式。无常论证是：若有我则我应当常住不变，而构成身心的五类要素（五蕴）没有一样是常住的，所以无我；控制论证从另一头入手：若真有一个我，人绝不会希望它改变。世亲一路的还原分析并不因此取消「人」这个说法——日常说话的层面（佛教称世俗）照样说「人」，它指的就是这五类要素的集合；只有在究极分析的层面（佛教称究竟）才把它还原成刹那生灭的诸要素。相续靠因果关联与业的条件维持，不靠一个持续的主体。',
        sourceIds: ['PI-2', 'PI-3'],
      },
    ],
    caution:
      '两边表面上都在谈「什么支撑同一」，目标却不同。持续问题要的是一条可判真假的条件：某个未来存在者究竟是不是数值同一意义上的我。佛教的无我分析属于一套关于修行与解脱的主张，它要说明的是执取一个常一之我如何产生苦，而不是给同一关系补一个更好的判准。所以不能把「无我」读成心理连续论或还原论的东方版本：当代的还原论（帕菲特一路）仍在回答持续问题，佛教则同时主张这个问题在究竟层面上问错了。反方向也要小心：无我并不否认日常人格、记忆或责任，佛教文本在世俗层面照样使用「人」。',
    pending: [
  ...
  },
  {
    ...
    why:
      '两场争论都从同一个结构性麻烦生出：如果每一次认知都要另一次认知来作保，保证就无穷后退；如果不要保证，又得说明可靠性从哪里来。分家就分在这里。',
    nodeIds: ['pt-nyaya', 'pt-knowledge-sources'],
    framings: [
      {
        tradition: '印度的「一次认知能不能自己作准」之争（svataḥ／parataḥ-prāmāṇya）',
        nodeId: 'pt-nyaya',
        caresAbout: ['一次认知凭什么算数', '要保证就无穷后退', '知不知道自己知道', '来源识别与事后复查'],
        note:
          '争点是一次认知的「量性」（prāmāṇya）从哪里来。古典印度把能生成真认知的合格途径叫作「量」（汉译佛典的用法：知觉是现量，推理是比量）；量性就是一次具体的认知够不够格算作「量」——它是不是由这样一个途径生成的、算不算真知。弥曼差与吠檀多一方由后退威胁推出知识自我证成（svataḥ prāmāṇya）：一次认知的可靠性由它自身带来，不需要另一次认知去确认。正理一方持外在确立说（parataḥ prāmāṇya）：一次认知不会自己知道自己为真——按这一派的说法，「知道 p」并不自动带来「知道自己知道 p」——所以量性要靠事后复查、辨认来源一类程序才能确立。',
        sourceIds: ['CR-1'],
      },
      {
        tradition: '当代内在论／外在论之争',
        nodeId: 'pt-knowledge-sources',
        caresAbout: ['理由要不要落在当事人这一侧', '心理状态说与可通达说', '证成为什么值得要', '被彻底欺骗的人算不算有理由'],
        note:
          '这场争论问的是：一个信念有没有理由（证成），取决于哪一类事实？说证成「随附于」某一类事实，意思是两个人在那一类事实上完全一样，他们的信念有没有理由也必定一样。内在论主张这一类事实全在当事人内部，外在论否认（大陆文献多把这两派作内在主义／外在主义）。「内部」有两种读法：心理状态说把它定为当事人心理生活的相关方面，可通达说把它定为当事人一经反思或内省就能知道的条件。支持外在论的主要理由是：证成之所以值得要，是因为它让信念更可能为真，而这一点靠的是来源在事实上可靠，不是当事人心里清不清楚；此外它解释力更强。支持内在论的主要理由包括证成的规范性，以及一个直观——被恶魔彻底欺骗的人，其信念与我们同样有理由，因为从内部看他做得一点不差。',
        sourceIds: ['CR-2'],
      },
    ],
    caution:
      '后退结构是共用的，被追问的东西却不是一件。印度这一场的直接问题是「知不知道自己知道」：正理否认的是「知道 p 就自动知道自己知道 p」，争的是一次认知的量性怎样被确立、被认定。当代这一场问的是一个信念有没有理由取决于什么，而这里的「内」被定为心理状态，或一经反思即可知道的条件，与「自己意识得到」并不重合。所以「正理＝外在论」这个对应会误导：在量性问题上持外在确立说，完全可以同时在证成问题上是内在论者，反过来也一样。要把这个对应落实，必须逐项说明两边的「内／外」各指什么——本条没有做到这一步，只指出两者共用同一个后退难题。',
    pending: [
      '正理内部（Vātsyāyana、Udayana、Gaṅgeśa）对量性确立程序的具体差别未查证，本条只用了综述对学派立场的概述。',
      '弥曼差 svataḥ prāmāṇya 的论证形式（Kumārila 与 Prabhākara 的差别）未核对。',
      '未核对把这两场争论直接对接的比较研究；上面关于两者如何不同的判断是本库的解读，不是引自来源的结论。',
    ],
    sources: [
      {
        id: 'CR-1',
  ...
  },
  {
    ...
    framings: [
      {
        ...
        note:
          '顺世论常被简化成「只承认亲眼所见」，可核对的陈述要细一层：不是所有种类的推理都有问题，为了过日常生活，推理是必不可少的一步。他们把推理分成两类——依概然作出的和依确定作出的；烟从屋顶升起、因而屋里很可能有火，就是前一类的常用例子。被拒绝的只有一种：「关于从未被知觉之物的推理，即神与来世」。这一栏的证据基础本身很薄：顺世论没有教义文本传世，连据称属于它的根本典籍《Bṛhaspati Sūtra》也只剩零星残片。',
        sourceIds: ['UO-1'],
      },
      {
        tradition: '休谟的归纳问题',
        ...
        note:
          '这一支追问的是归纳暗中预设的那条齐一原则：「未经验的事例必与已经验者相似，自然的进程始终齐一」——说白了就是没见过的情况会和见过的一样，自然不会中途改规矩。两难的形式是：论证式推理（结论的反面会引出矛盾的那一类）不能确立它，因为「自然改了规矩」并不自相矛盾，人完全可以清楚地设想已有规律在未观察的情形中不再成立；概然推理也不能，因为一切概然推理本身都已「预设将来将与过去一致」，用它来证明齐一原则，就是「把正待论证之点当作前提」。',
        sourceIds: ['UO-2'],
      },
    ],
    caution:
    ...
    pending: [
      '正理一方对遍充（vyāpti，即「凡有烟处必有火」这类普遍伴随关系）如何被确立的正面论证，以及顺世论者对它的具体反驳，本轮没有找到可核对的条目段落。SEP Naturalism in Classical Indian Philosophy §2.3 只讨论 svabhāvavāda 与因果，不涉及推理的量性，因此没有采用。',
  ...
  },
  {
    ...
    framings: [
      {
        ...
        note:
          '中世纪几乎所有作者都同时肯定神全知与人自由，问题因此是内部的：若神已知我将如何行动，这些行动如何还能是自由的——看来我并不能有别的意愿或别的做法。波爱修不把这个难题当成单纯的逻辑谬误，而提出另一条思路：一件事被认知成什么样，取决于认知它的是什么样的存在者，而不取决于那件事本身已经定到什么程度——按他的说法，被认知者是按认知者的能力被把握的，不是按它自身的分量被把握的。照这个说法，未来偶然事件在它自己那一层仍然是未定的。阿奎那沿此路走。奥卡姆与司各脱另走一路，主张神必须偶然地意愿，并因此拒绝把必然性等同于时间上的恒常。伊本·西那与伊本·鲁世德（拉丁世界称他们阿维森纳与阿威罗伊）把神的知识限于普遍者（一般的、可重复的东西，而不是一件件具体事情）以避开难题；迈蒙尼德则诉诸神的不可知，并主张「知道」一词用在神身上与用在人身上并不是同一个意思。',
        sourceIds: ['FK-1'],
      },
      {
        ...
        note:
          '当代重构把这个推理排成十步，骨架分三段。先是两条前提：神在很久以前就无误地相信某个行动将发生；而过去的事仅仅因为它已经过去就已必然——谁也改不了昨天。两条合起来得出：神当年的那个信念是必然的。第二段加一条规则：必然性可以从前件传到后件（前件必然，而且有前件就必有后件，那么后件也必然）。既然「神如此相信」就必定「该事发生」，该行动因此现在就是必然的。第三段收口：必然之事不能不做，所以行动者不能不做；再加上「没有别的可能就没有自由」，得出该行动不自由。排成十步之后，回应无非是挑其中一步来否认：永恒方案否认「很久以前就相信」这句话适用于神，因为神并没有处在时间中的信念；软事实方案则区分纯粹关于过去的硬事实与其实有一半关于未来的软事实，并把神对未来偶然之事的信念归入后者。',
        sourceIds: ['FK-2'],
      },
      {
        tradition: '世俗自由意志争论',
        ...
        note:
          '这一栏把同一副骨架放在没有神的版本里。后果论证由过去的固定性与自然律的固定性出发，经同一条传递规则得出：若决定论为真，则「对任何真命题 p，没有人对 p 是否成立有过任何选择」。这也解释了为什么两边的回应形式相似——或否认必然性可以这样传递，或否认自由需要替代可能。',
        sourceIds: ['FK-3'],
      },
    ],
    caution:
    ...
    pending: [
      ...
      '「中世纪与跨文化传译」与「因果、自由与责任」两页之间的互链尚未补上：读者只能从这一条比较跳过去，两页正文之间还接不上。',
    ],
    sources: [
      {
        id: 'FK-1',
  ...
  },
  {
    ...
    framings: [
      {
        ...
        note:
          '亚里士多德走功能论证，落点在活动而不在状态：幸福是灵魂理性部分依德性而进行的活动，而且要终身良好地运用理性才算。伦理德性不是一门知识，是由习惯养成的品性——幼时先养成习惯，理性成熟之后才谈得上实践智慧。这条路也没有把快乐和外在条件逐出去：幸福的生活必须包含快乐，而完满的友谊需要外在资源与时间，于是这些也进入幸福的条件。',
        sourceIds: ['GL-1'],
      },
      {
        tradition: '伊壁鸠鲁的宁静',
        ...
        note:
          '伊壁鸠鲁把目标定在心的不受扰动：对死亡等的恐惧一旦消除，剩下的就是心的宁静（ataraxia），它与身体无痛一起构成人生目标的一种表述。关键的区分是动态快乐与静态快乐：前者伴随某个正在进行的过程（如闻到宜人的气味、听到悦耳的声音），后者是身心安好这一状态本身所带的快乐。由此得出快乐不因持续时间而增加：完全免于身体痛苦已经到顶，再久也只是这个状态持续得更久。',
        sourceIds: ['GL-2'],
      },
      {
        tradition: '斯多亚的唯德是善',
        ...
        note:
          '斯多亚把好生活的内容压到一点：唯有德性是善——只有它使人受益；健康、财富、名声既不使人受益也不使人受损，因而非善非恶。但主流斯多亚不让所有无关者等价——健康、财富、名声属被优选的一类，合乎自然，选取它们因而是恰当的。综述也指出这产生解释困难：某物既与幸福无关，又值得理性地追求，看来自相矛盾。所谓赞同，是指事情一出现，心里会自动冒出一个判断——「这是件坏事」「这该发怒」——赞同就是接不接下这个判断。由于德性只取决于赞同，而赞同在我们能力之内，幸福「也将在行动者的能力之内、完全由他决定」。',
        sourceIds: ['GL-3'],
      },
      {
        tradition: '当代福祉理论',
    ...
    caution:
      '把德性、宁静、快乐排成三个并列的答案，走得太快。三个古典学派不只在答案上分歧，在「答案要满足什么」上也分歧：亚里士多德要一个能指导终身活动与教育的人的善；伊壁鸠鲁要一套解除恐惧与欲望的疗法，快乐的上限就在痛苦解除处，因此它不需要一个可以无限累加的量表；斯多亚要一个使幸福完全不受运气支配的方案，代价是把健康与财富逐出「善」的范畴，而这个代价在他们自己的体系里也留下了一处解释困难。当代福祉理论问的又是别的东西——「对我好」这一关系，它明确把道德上的好与对我而言的好分开，而古典方案通常不接受这个切分。所以不能把「德性论」直接读成一种客观清单论。',
    pending: [
  ...
  },
  {
    ...
    question: '「法」要解决的秩序问题，和现代法治要解决的问题是同一个吗？',
    why:
      '两边都主张用一般的、公开的规范取代因人而异的裁断，给出的理由也相似：靠在位者的个人能力不可靠。既然出发点这么近，差在哪里就值得逐项看——而差别落在这套规范首先约束谁。',
    ...
    framings: [
      {
        ...
        note:
          '「法」是一整套非人格的规范、法律、条例与制度方案，它要解决的是国家怎么治理的问题。君主的位置最能看出这套规范朝哪个方向用力：他有权、甚至有责任在情势要求时更改法，同时被极力劝阻不要对现行法作任性干预。综述还指出这一传统的一个悖论——制度被设计成即使君主不贤不智也能运转。',
        sourceIds: ['FA-1'],
      },
      {
        tradition: '现代法治理想',
        ...
        note:
          '现代法治的分界线正好落在上一栏留空的地方。以法而治（rule by law）几乎不给统治者加上任何法律义务——国家用法来控制公民，却尽力不让法被用来控制国家；法治则要求对法的义务同等地适用于统治者与被统治者。形式要件包括一般性、公开性、不溯及既往、可理解、一致、可行、稳定，以及吻合——即官员的实际作为要与公布的规则相符；程序要件包括独立公正的裁判、律师代理、在场与质证、听取裁判理由。还有一处不对称：对普通人，未被明文禁止者即被允许；对国家与官员，则可能采取相反的推定——只能依明确的法律授权行事。是否把人权一类实质内容算入核心要件，本身仍有争议。',
        sourceIds: ['FA-2'],
      },
    ],
    caution:
      '共同点是真的，差别落在被保护的对象上。「法」传统的规范首先向下和向内运作——约束民与官，使国家可治；现代法治的定义性要求则是把义务同等地施加到统治者自己身上。所以「法家＝早期法治」是错的；但只写这一句否定也不够，因为「以法而治」这个概念恰好说明二者共享的是制度技术，分歧在于法对权力的方向。还要接住综述自己的提醒：「Legalist」这个标签会把研究窄化成一个问题——它与西方的 rule of law 是什么关系；一旦只从这个角度看「法」，就会漏掉这套思想对官僚考核、赏罚与君主自我抑制的关切，而那些才是它的主要议题。',
    pending: [
      '《商君书》《韩非子》相关篇目原文未核对，「法」一栏依据综述。',
      '「君主是否受法约束」在综述 §5.2 被写成一种悖论式紧张（君权在理论上无限，实际上被制度设计抑制），本条转述这一紧张，未采纳某种解读。',
      '中国近代以来「法治」译名与 rule of law 的关系（综述提到自梁启超起的讨论）没有核对中文文献。',
      '本条只对照「法」与法治，没有处理儒家「礼」与二者的三方关系；那需要另立一条可比问题。',
    ],
    sources: [
      {
        id: 'FA-1',
        ...
        checkedOn: '2026-09-11',
        supports:
          '「法」指一整套非人格的规范、法律、条例与制度方案，被认为比诉诸在位者个人能力更可靠；君主有权且有责在情势要求时更改法，同时被劝阻不要任性干预现行法；§5.2 的悖论式紧张（制度须能容纳不贤不智之君）；条目对「Legalist」标签与只从 rule of law 角度研究「法」的警告。',
      },
      {
        id: 'FA-2',
  ...
  },
  {
    ...
    framings: [
      {
        ...
        note:
          '这一支明确把人格当作成就：一个个体可以是人类而仍然不是人格。人格是要去达成的地位，达成的办法是履行由自己的身分所界定的那些义务；达没达成，看的是习惯性行为养成的品性。这一栏所据的条目在卷首引了非洲哲学家门基蒂（Ifeanyi Menkiti）的一句话：人格是必须去达成的那种东西。条目还提出一个更强的主张：在这种道德里义务胜过权利，而不是相反，并把它与西方社会的道德体系对置——这是撰稿人本人的比较判断，不是学界的中立描述。',
        sourceIds: ['PH-1'],
      },
      {
        ...
        note:
          '这一支先把问题拆开，其中两问与这里最相关。人格问题：算作一个人格，而不是一块石头或一台机器，究竟是怎么一回事——我们身上有什么，是那些不算人格的东西所没有的？持续问题：一个人格要从这一刻持续到下一刻，需要什么？条目强调这两问大体上互相独立。要注意这里的人格问题要的是「人格」一词的定义与适用条件，答案是「算」或「不算」，不是一个可以做得更好或更差的地位。',
        sourceIds: ['PH-2'],
      },
    ],
    caution:
    ...
    pending: [
      'SEP African Ethics 由吉耶凯（Kwame Gyekye）撰写，非洲共同体主义伦理学一栏里那两条主张都出自他本人。要区分门基蒂的强共同体主义与吉耶凯自己的温和共同体主义，必须核对门基蒂 1984 与吉耶凯 1997 的原文，本轮没有做——因此「人格、共同体与 Ubuntu」页上的三条立场目前仍缺各自的独立来源，不能用这一条来支撑其中任意一条。',
