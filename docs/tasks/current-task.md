# 当前任务：哲学人物按传统与历史重构（2026-09-25）

状态：已提交、推送 origin/main 并部署正式 Worker；线上复验完成（2026-09-25）。包含上一轮人物入口及本轮历史结构重构。

## 需求、验收与计划

- 用户指出人物建设缺少逻辑：按东西方及更多传统分组，按当地历史分期，展示学派／方法与关键内容。
- 将姓名索引降为辅助，默认展示五组、阶段导语、人物与明确缺口；保留旧人物锚点与阅读映射。
- 给既有人物补时代、学派限定和来源；补足关键断层，每个新增入口包含理由、边界、文本定位、问题与历史链接。
- 原生链接与 details；手机单列，无新增客户端交互。执行 check、build、链接与浏览器验证。

## 基线与顾问协作

- HEAD e4033d0；工作树已有上一轮 12 人索引及配套改动，尚未发布。本轮在其上继续，不将它当作生产事实。
- 用户显式调用 codex-chatgpt-collaboration。一次发送中文摘要，无附件或私有文件；当前可见模型 6 Pro，22 秒空输入基线。正文 882 码点／23 行，SHA-256 1a2fa60928bf708b4c6c443a7fa2e62ad22dab99945b97d99c6cd1321aacd6ee；发送前比对通过，发送后唯一用户正文出现、输入框清空。
- 完整顾问意见已收到，停止生成且出现回复操作。采纳五组本地分期、空段显式缺口、学派限定、原生 details 与旧锚点保留；具体人物和事实由 Codex 独立核对 SEP／IEP。对话地址不入库。
- 资料核查使用 SEP 各人物条目及 IEP 牟宗三条目；只核验所列章节，不宣称原典全文、译本或全站复审完成。

## 发布授权与执行

- 用户明确要求“提交 部署 上线”。按项目规则完成提交、推送 origin/main、部署 Worker 及线上复验。
- 发布前已 fetch origin，main 无额外待推送历史提交；本次包含人物入口首版及历史重构全部改动，无数据库迁移。
- 发布前重新执行 pnpm check 与 pnpm build 均通过。内容提交 3051d3d 已推送 origin/main，随后 pnpm deploy:only 成功；无远程 D1 迁移。
- 正式 Worker 版本 b074c321-0568-4b1f-b75a-9aabcba83e37，上传 10 个新资源，启动时间 39ms。
- 正式域名 https://killua.win 的浏览器逐页复验 60 个学习入口（包括旧 philosophy-tree 入口）：标题与正文可读，826 处跨页／页内锚点有效，无错误页；人物页 25 张卡、5 组历史线，龙树原生 details 键盘展开成功。
- 命令行直连正式域名返回 403，线上证据采用正常浏览器 DOM 与交互，不将其写成 HTTP 全量 200 或完整视觉回归。此次没有重新做生产性能专项测试。
- 发布记录以独立文档提交同步 origin/main；相对内容提交仅修改本文件，线上运行时代码与 main 一致。

## 实施与验证证据

- 五组、22 个阅读阶段、25 人；原有 12 人保留稳定锚点，新增 13 人。14 人接既有问题精读（22 处精确映射），11 人为带学术来源的历史概览；概览的问题对读不冒充目标页人物专段。
- 新增 people-history.ts（分期与缺口）、people-metadata.ts（时代、学派限定与逐人来源）、people-profiles.ts（历史概览、边界、导读与比较）。人物注册表统一生成问题和传统反链，轻量首页仅导入分组及身份数据。
- 五组分别为中国 6 人、印度与南亚 2 人、西方 13 人、伊斯兰世界 2 人、非洲与非裔 2 人。各自分期，人物比较不写成影响链；姓名索引为辅助 details。
- 新增完整性检查覆盖分期存在、空段缺口、历史元数据、问题／传统连接与对读目标；原精读来源状态逐引用验证。未改数据库、55 节点树及既有问题结构。
- 最终 pnpm check 通过：Lint、TypeScript、16 个迁移本地回放、9 模块／20 项不变式；pnpm build 五阶段完成。vinext 仍有部分路由静态分类 Unknown 提示，不等于构建失败，也不宣称完整视觉回归。
- HTTP 检查 59 个学习路由均正常、826 处页内链接目标有效、无重复 ID；25 人全部登记问题与历史节点反链存在，含新增概览 questionNodeId。
- 浏览器检查桌面首页五组入口、320px 首页到西方历史锚点、360px 印度分期与龙树导读。实际内容宽度分别 305px／345px，scrollWidth 与 clientWidth 相同，无横向溢出；Enter 展开原生 details 成功。
- 禁用脚本后重新加载人物页，以原生 UI 展开龙树卡，理由和来源可读；已恢复脚本与视口。无新增客户端组件或交互脚本，不声称框架不注入脚本。
- git diff --check 通过。来源核对限定于列出的学术综述章节和原有来源账；11 个概览不是原典全文核验或完整研究页。秦汉隋唐、清代、印度近现代、伊斯兰后古典／近现代等具体缺口可见，尚未独立复审。
- 顾问完整答复两次读取一致，生成结束与回复操作已确认；保留协作标签，本轮没有再发送或上传。
- 开发完成时人物页面仅在本地；随后经用户明确发布授权，现已上线，版本及线上证据见上方发布记录。

---

# 历史任务：哲学「按人物」阅读入口（2026-09-25）

状态：**本地首版完成，验收通过；未提交、未推送、未部署。** 本轮用户要求分析并完善第三种阅读入口。

## 需求与验收

- 保留问题与传统两轴，增加人物入口，连接具体问题、理由、边界、文本与历史语境。
- 首版采用轻首页与独立人物索引，原生链接和 details，不批量建设薄弱人物详页。
- 使用精确人物 ID 与正文映射，不按人名字符串命中推断思想关系；旧路由与旧锚点保留。
- 所有正反向链接、来源 ID、文本与论证定位有效；核验状态沿用逐条原记录，不新增“已核验”宣称。
- 执行 pnpm check、pnpm build、HTTP 路由及锚点检查，浏览器窄屏与键盘验证。未执行的验证不得宣称通过。

## 基线与方案判断

- 工作树干净，HEAD / origin/main 为 e4033d0；正式 Worker 仍为上一轮已发布版本。
- 浏览器打开正式 /learning#learning-philosophy：当前只有按问题与按传统入口。
- 精读数据已有 philosopherViews（framing / application / caution / sourceIds）与 texts，可复用；正文提及人名不等于有可用人物阅读链。
- 选择 12 人：亚里士多德、笛卡尔、休谟、康德、孟子、朱熹、安萨里、波伏瓦、杜波依斯、吉利根、维特根斯坦、伊壁鸠鲁。按现有证据选择，不是排名或完成度。
- 首页只展示孟子、笛卡尔、波伏瓦三个不同问题起点；全索引按中文名拼音排序。历史语境为多值阅读链接，不是互斥学派分类。
- 暂不把现有待核验的罗尔斯、库恩阅读材料升级为正式人物入口；印度及非洲大陆人物的材料链是优先缺口。人物名称、集体文本、传世文本的归属须逐条辨别。
- 第一版不新增生卒年、原文名或影响关系；已有正文长名作为查找辅助。以后新增异译和同名人物须有身份依据与明确消歧。

## 顾问协作证据

- 用户显式调用 codex-chatgpt-collaboration；仅发送 2 项问题的中文任务摘要，无附件、代码、日志或私人文件。对话地址只保留在本地会话上下文，不写入仓库。
- 新任务独占标签，3 次空输入核对跨度约 17 秒；规范输入 877 码点、29 行，任务标记 f9872b50239edce3，SHA-256 58c3fe632158d4536d5fac605aa8748e901a105ef8ad657b9c010d28521eedab。提交前正文、哈希、标记及附件清单一致；当前可见模型 6 Pro。
- 首次点击发送无可见效果；同一对话 3 次确认无用户消息、无生成，跨度超过 5 秒，编辑器与原文稳定，随后唯一一次 Enter 兜底。浏览器无服务端幂等键；最终用户正文完整精确出现 1 次、用户消息 1 条，没有重复。
- 20 分钟完成上限内收到完整答复；停止按钮消失、回复操作出现，两次正文相同，中间检查页面状态。标题 codx-哲学人物入口设计 已可见确认；状态 CONNECTED。
- 顾问建议轻入口 + 独立索引，首批最多 12 人，原生 details，稳定段落锚点，同一映射生成反链，来源逐项核验，缺口公开说明。已采纳。
- Codex 本地确认结构与内容映射；保留既有来源日期，未将顾问抽查网站当作外部原典复核。未上传文件、未扩大发布授权。全文复制改为复用既有字段，不另写一套人物简传。

## 本地改动与验证

- 新增 `/learning/philosophy/people`：12 人、20 处精确论证／文本映射、12 处历史语境映射；人物卡含具体问题、复用理由、边界、分步阅读、来源定位与收录缺口。首页新增 3 张推荐卡，总览增加人物入口，站点地图登记新路由。
- 问题页与历史页共用尾部生成反链；人物论证与文本新增稳定锚点，保留旧锚点。两条手写教学主线仍走共享渲染，浏览器额外确认 mind-self 中笛卡尔段落与人物反链均存在。
- 最终 `pnpm check` 全部通过：Lint、TypeScript、16 个迁移本地回放、9 模块／20 项哲学不变式。开发中锚点函数参数过宽造成类型错误，已收窄为只需人物 ID，最终检查通过。
- 最终 `pnpm build` 五阶段通过。vinext 仍提示部分路由静态分类 Unknown；构建成功不等于全站视觉回归。
- 本地 HTTP 检查：59 个学习路由均 200，687 处页内链接目标有效，未发现重复 ID；12 人在全部登记问题与历史页的反链存在。
- 浏览器桌面检查人物索引；360px 检查人物卡和学习首页第三入口，`scrollWidth === clientWidth === 345`（扣除滚动条），无横向溢出。Enter 展开原生 details 成功。
- CDP 暂停脚本并重新加载人物页后，孟子卡的正文、原生展开、来源状态与文本指导仍可读取；原生 UI 点击可进入孟子论证段落。已恢复脚本执行并复原视口。无新增客户端组件或交互脚本；开发框架自身仍注入脚本，不声称 HTML 完全没有 script 标签。
- `git diff --check` 通过；实际来源未全量重审、原典版本与译者未补齐、未独立复审、未进行生产部署或线上性能验证。当前生产站仍是上一轮发布版本。
- 自审发现与未来收录门槛同步到 `CONTENT_GAP_REVIEW.md`，人物映射职责同步到知识库规则与架构说明。

---

# 历史任务：哲学人物与学派缺口补充（2026-09-24）
